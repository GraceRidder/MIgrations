...
q <- timeSliceTree(s, 800, plot = F) ### from the tips?

gammaStat(drop.extinct(tree))

rw

rw <- treeSlice(s, 200,  trivial = F) ### from the root?
...
plot(drop.extinct(rw[[2]]), show.tip.label = F)
axis(1)
gammaStat(drop.extinct(rw[[2]]))
...
plot(dropExtinct(myTree), show.tip.label = F)
