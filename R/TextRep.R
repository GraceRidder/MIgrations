...
print(paste( "sites:",sites, " replicates:",reps))

## counting time steps before extinctions
timelen <- c()
for (i in 1:reps){
timelen <- append(timelen, length(rs[[i]]$reslist[[1]]$mig))
}


print(paste("time steps for each site to go extinct:"))
print(timelen)

e <- 0
for ( i in 1:reps){
  if (sum(unlist((rs[[i]]$reslist[[1]]$matrix_list))) == 0){
    e <- e+1
  }
}

print(paste(e, "sites extinct"))
...
for (site in 1:sites){
  ...
  for (rep in 1:reps){
    ...
    while (length(tpac) < 1000) {
      tpac <- append(tpac, 0)
    }
    while (length(apac) < 1000) {
      apac <- append(apac, 0)
    }
    X <- cbind(X, tpac)
    Xx <- cbind(Xx, apac)
